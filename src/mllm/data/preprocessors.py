...
def parse_llm_output_as_json(
    ...
    if "```" in cleaned:
        try:
            ...
            for block in reversed(code_blocks): # Check blocks from bottom up (usually the final answer)
                try:
                    return json.loads(block, strict=False)
                except json.JSONDecodeError:
                    # Try brace extraction on the block itself
                    try:
                        b_start = block.index("{")
    ...
    if not (cleaned.startswith("{") and cleaned.endswith("}")):
        try:
            # Find the LAST occurrence of a { ... } pair to skip templates at the top
            import re
