...
def parse_glossary(
    ...
    if repo_root is not None:
        for size in [36, 51, 60]:
            candidates.append(repo_root / "ontology" / "glossary" / subject_upper / f"{subject_lower}-{size}-reference.md")

    # Fallback to package parent directory search
    parents_root = Path(__file__).resolve().parents[3]
    for size in [36, 51, 60]:
        candidates.append(parents_root / "ontology" / "glossary" / subject_upper / f"{subject_lower}-{size}-reference.md")
