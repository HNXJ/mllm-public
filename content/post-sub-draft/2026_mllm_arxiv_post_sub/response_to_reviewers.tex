...
\item \textbf{Passage-Level Grounding}: We conducted a 27-cell passage-support adjudication across prespecified diagnostic disagreement and failure-mode cells, verifying scored claims against verbatim text from the canonical PDFs to confirm that anti-guess rules prevent speculative inferences (Supplementary Table S7).
\item \textbf{Human--Council Concordance}: Across 175 non-null paired factor cells nested in 25 papers, human expert consensus and autonomous LLM council consensus achieved $93.14\%$ all-cell sign agreement ($163/175$) and $97.02\%$ strictly non-zero directional concordance ($163/168$; $\text{MAE} = 0.301, r = 0.162, \rho = 0.241$; Supplementary Table S7 and Fig.~S2). At the study-level aggregate ($N=31$), human-to-council correlation was $r = 0.390$ ($\rho = 0.360$), matching inter-human rater agreement ($H_1$ vs $H_2$: $r = 0.390, \text{MAE} = 0.308, 88.96\%$ all-cell sign agreement).
\end{enumerate}
...
\authorsresponse We conducted a $31\text{ papers} \times 3\text{ models} \times 3\text{ temperatures } (T \in \{0.00, 0.35, 0.70\}) \times 3\text{ repeats}$ factorial robustness experiment (837 inference calls; Supplementary Table S8 and Fig.~S3):
\begin{enumerate}[leftmargin=*,noitemsep]
...
\authorsresponse We performed a comprehensive subgroup sensitivity analysis across Study Type, Modality, Era, and Publication Status (Supplementary Table S11), alongside controlled evidence ablations (Supplementary Table S9):
\begin{itemize}[leftmargin=*,noitemsep]
\item \textbf{Subgroup Heterogeneity}: Rather than uniform invariance, the local-to-global displacement is heterogeneous across corpus strata: Human In Vivo studies ($N=7$) showed a negative displacement ($\Delta_{\text{LO-GO}} = -0.059$), whereas Animal In Vivo electrophysiology ($N=5$) exhibited a positive displacement ($\Delta_{\text{LO-GO}} = +0.087$). Studies from 1982--2014 ($N=12$) and 2015--2020 ($N=7$) exhibited negative displacements ($-0.024$ and $-0.041$), whereas recent studies (2021--2025, $N=12$) exhibited positive displacement ($+0.038$). All 31/31 papers are peer-reviewed publications.
\item \textbf{Evidence Grounding Control}: Across 54 inference calls, when evidence text was shuffled (Negative Control $H_0$), context displacement collapsed to $\Delta_{\text{LO-GO}} = -0.004$ (compared to $+0.038$ for intact text), and under prior-only prompts without text, displacement collapsed to $\Delta_{\text{LO-GO}} = +0.000$ (Supplementary Table S9 and Fig.~S4), demonstrating that scoring is sensitive to the supplied empirical text.
\end{itemize}
...
\authorsresponse We normalized all bibliographic citations and in-text references to Westerberg \& Xiong (2025) consistently throughout the manuscript and Supplementary Information.
...
\begin{itemize}[leftmargin=*,noitemsep]
\item We performed a 10-figure manual expert validation against primary source PDFs, achieving a 10/10 overall PASS (directional concordance: 10/10; ontology mapping: 9 correct, 1 partial; numerical extraction: 9 correct, 1 minor error; Supplementary Table S7).
\item In a full 31-paper figure-text ablation sweep (533 paired factor cells), removing figure-derived text blocks preserved the direction of the corpus-level $\text{LO}-\text{GO}$ contrast ($\Delta_{\text{Full}} = +0.0408$ vs $\Delta_{\text{NoFig}} = +0.0565$) while producing measurable paper-level score adjustments ($r = 0.8539, \text{MAE} = 0.2253, 99.42\%$ directional concordance on non-zero cells [518/521]; Supplementary Table S10 and Fig.~S5), with Westerberg \& Xiong (2025) remaining a focal divergent outlier in both conditions (Rank 28/31 Full vs Rank 29/31 NoFig).
\end{itemize}
