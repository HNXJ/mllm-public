...
d Mismatch [MASKED_JOURNAL]s balance excitatory motor-related input

against visual inhibition

d Visual inhibition onto mismatch [MASKED_JOURNAL]s is mediated by

somatostatin inter[MASKED_JOURNAL]s
...
Alexander [MASKED_AUTHOR], Bo Wang,
[MASKED_AUTHOR]
...
[MASKED_AUTHORS], 2017, Cell 169, 1291-1302
...
Alexander [MASKED_AUTHOR],1,2,3 Bo Wang,1,2,3 and [MASKED_AUTHOR]1,2,4,*
1[MASKED_INSTITUTION], Maulbeerstrasse 66, 4058 Basel, Switzerland
...
atory and inhibitory L2/3 [MASKED_JOURNAL]s in response to tran-
sient visuomotor mismatches in both groups of mice.
Mismatch responses in excitatory [MASKED_JOURNAL]s were
strongly experience dependent and driven by a tran-
sient release from inhibition mediated by somato-
statin-positive inter[MASKED_JOURNAL]s. These data are consis-
...
response patterns of [MASKED_JOURNAL]s in V1 give surprisingly poor reflec-
...
pretation comes from the finding that a subset of [MASKED_JOURNAL]s in V1
...
[MASKED_JOURNAL] computes a difference between an inhibitory visual input
...
inter[MASKED_JOURNAL]s in visual cortex, we show that this visual inhibition is
likely mediated by somatostatin (SST) inter[MASKED_JOURNAL]s. Finally, we
...
Mismatch Responses in Excitatory [MASKED_JOURNAL]s Depend on
Visuomotor Experience
...
[MASKED_JOURNAL]s express the red fluorescent protein tdTomato, which
allowed us to restrict analysis to identified excitatory [MASKED_JOURNAL]s.
In these mice, we found that 96.8% ± 0.7% (mean ± SEM) of
GCaMP6f labeled [MASKED_JOURNAL]s were excitatory (Figures S1C and
S1D). Thus, for all following analysis we pooled putative excit-
atory [MASKED_JOURNAL]s of the C57BL/6 mice and the identified excitatory
[MASKED_JOURNAL]s of the vGAT-Cre 3 Ai14 mice. In total, we recorded from
2,259 excitatory [MASKED_JOURNAL]s in CT mice (996 putative excitatory and
1,263 identified excitatory [MASKED_JOURNAL]s) and 2,104 excitatory neu-
rons in NT mice (764 putative excitatory and 1,340 identified
excitatory [MASKED_JOURNAL]s).

We found that in CT mice, a considerable fraction of excitatory
[MASKED_JOURNAL]s responded to mismatch (865 of 2,259 [MASKED_JOURNAL]s or 38.3%;
...
et al., 2012). In NT mice, the fraction of [MASKED_JOURNAL]s that responded
to mismatch was smaller (425 of 2,104 [MASKED_JOURNAL]s or 20.2%), and
...
back halt (Figure 1E), and individual [MASKED_JOURNAL]s often responded
to both mismatch and playback halt (Figures S1F and S1G).
With increasing mismatch response, [MASKED_JOURNAL]s in CT, but not NT,
...
responsive [MASKED_JOURNAL]s increased with average amplitude of the
mismatch response. On average mismatch [MASKED_JOURNAL]s responded
to 37.5% of mismatches in CT mice and to 33.8% in NT mice
(Figure S1I). A subset of [MASKED_JOURNAL]s responded with a decrease in
...
[MASKED_JOURNAL]s decrease their response.
...
Figure 1. Mismatch Responses in Excitatory [MASKED_JOURNAL]s Depend on Visuomotor Experience
...
(C) Sample fluorescence traces (DF/F, black lines) of an excitatory [MASKED_JOURNAL] in a CT (left) and a NT (right) mouse, during a closed-loop (top traces) and an open-loop
...
mice, we found [MASKED_JOURNAL]s that selectively respond to mismatch, whereas in NT mice, [MASKED_JOURNAL]s that responded to mismatch also responded to corresponding
...
(D) Average mismatch response (DF/F) of all [MASKED_JOURNAL]s in CT mice (left, nine mice, 2,259 [MASKED_JOURNAL]s) and NT mice (right, nine mice, 2,104 [MASKED_JOURNAL]s), sorted by amplitude
...
Methods). Orange bar marks the duration of mismatch. In CT mice, the fraction of [MASKED_JOURNAL]s with a significant mismatch response was larger than in NT mice
...
> Figure caption (from PDF text): Figure 1. Mismatch Responses in Excitatory [MASKED_JOURNAL]s Depend on Visuomotor Experience
(A) Experimental timeline. Mice were dark-reared from birth. AAV injection and imaging window implantation occurred on postnatal day 30 (P30). From P32 to
P42, mice had six training sessions in coupled (coupled trained [CT]), non-coupled (non-coupled trained [NT]), or dark (dark trained [DT]) conditions, followed by
two to five imaging sessions beginning at P44 and spaced by 2 days. Some of the mice were put on a normal 12-hr/12-hr light/dark cycle after the second imaging
session.
...
This figure is composed of six panels (A through F) illustrating the experimental timeline, training setup, single-[MASKED_JOURNAL] traces, and population response data comparing two conditions: Coupled Training (CT) and Non-coupled Training (NT).

## 

explain mismatch responses in a layer 2/3 excitatory [MASKED_JOURNAL]
would be that such a [MASKED_JOURNAL] integrates an excitatory motor-
...
[MASKED_JOURNAL]. If this is correct, mismatch [MASKED_JOURNAL]s should receive excit-
atory motor-related input and inhibitory visual input. To test this,
we computed the correlation of the activity of each [MASKED_JOURNAL] with
visual flow and with running speed during the open-loop ses-

sions. As running and visual flow are independent in open-loop
sessions, the activity of a [MASKED_JOURNAL] that receives net inhibitory
...
correlations of all [MASKED_JOURNAL]s revealed that [MASKED_JOURNAL]s with a strong
...
ure 2B). When comparing the entire population of [MASKED_JOURNAL]s, we
found that, in CT mice, [MASKED_JOURNAL]s with a positive correlation with
running speed tended to have a negative correlation with visual
flow, whereas in NT mice [MASKED_JOURNAL]s with a positive correlation
...
(A) Circuit model in which an excitatory mismatch [MASKED_JOURNAL] (MM, gray) integrates excitatory motor-related input and inhibitory visual input relayed by a local
inhibitory inter[MASKED_JOURNAL] (orange) to compute the difference between predicted and actual visual flow.
(B) Correlation coefficients between neural activity (DF/F) of layer 2/3 excitatory [MASKED_JOURNAL]s with running speed and with visual flow in CT (left; nine mice) and NT
(right; nine mice) mice during open-loop sessions. Each dot represents a single [MASKED_JOURNAL] (CT: 2,259 [MASKED_JOURNAL]s; NT: 2,104 [MASKED_JOURNAL]s). Dot color indicates the amplitude
...
(D) Spiking output of a simple conductance-based leaky integrate-and-fire [MASKED_JOURNAL] (LIF) was convolved with a unitary calcium-kernel to simulate [MASKED_JOURNAL]al activity
...
(E) Sample fluorescence (DF/F) traces of two [MASKED_JOURNAL]s responding to mismatch from two mice during a closed-loop session (black traces) and the corresponding
...
mismatch are labeled as in (Figure 1C). Also shown is the FEV: fraction of explained variance (see STAR Methods) for each example [MASKED_JOURNAL].
(F) Lower left: distribution of the fraction of variance explained (2,259 [MASKED_JOURNAL]s), estimated as the squared correlation (R2) coefficient between model output and
calcium activity during the closed-loop session. Top right: average correlation map and average location of maxima (black crosses) for [MASKED_JOURNAL]s with significant
positive responses to mismatch, averaged over mice (nine mice, top 50% of significant [MASKED_JOURNAL]s per mouse).
...
layer 2/3 excitatory [MASKED_JOURNAL]s that are strongly activated by
...
excitatory [MASKED_JOURNAL]. We then predicted the activity of each excit-
atory [MASKED_JOURNAL] during the closed-loop session by using visual flow
and running speed of that session as inputs to the LIF model opti-
mized for the particular [MASKED_JOURNAL] (Figure 2E). We found that the
...
Methods) for excitatory [MASKED_JOURNAL]s with a significantly positive

response to mismatch and found that activity of these [MASKED_JOURNAL]s
...
atory [MASKED_JOURNAL]s can be explained by a combination of an excitatory
motor-related input and inhibition by visual flow. Consistent with a
visually driven inhibition of mismatch [MASKED_JOURNAL]s, mismatch respon-
sive [MASKED_JOURNAL]s exhibited a decrease of activity in response to the
onset of visual flow in open-loop conditions (Figures S3A-S3C).

SST Inter[MASKED_JOURNAL]s Decrease Activity during Mismatch
...
rons. These [MASKED_JOURNAL]s would have to be strongly driven by visual
...
probe the responses of different inhibitory [MASKED_JOURNAL] subtypes,
...
tide-Y (NPY) (Gong et al., 2007) inter[MASKED_JOURNAL]s. The SST-Cre, VIP-
Cre, and PV-Cre lines collectively target approximately 80% of
inter[MASKED_JOURNAL]s in mouse V1 and the labeled populations are largely
non-overlapping (Pfeffer et al., 2013).

We found that SST inter[MASKED_JOURNAL]s exhibited a higher correlation
with visual flow than other inter[MASKED_JOURNAL] subtypes or excitatory
[MASKED_JOURNAL]s (Figure 3). Moreover, of the four inter[MASKED_JOURNAL] subtypes,
only SST inter[MASKED_JOURNAL]s responded, on average, with a drop in
...
rience, as it was present in both CT (five mice, 118 [MASKED_JOURNAL]s) and
NT mice (five mice, 157 [MASKED_JOURNAL]s), indicating that the visual input
onto SST [MASKED_JOURNAL]s is established independently of motor-related
input. Locomotion strongly increased visual responses in SST
inter[MASKED_JOURNAL]s (Figure 4B), but running-onset responses were
almost completely absent in darkness (Figure S3D), consistent
with a predominantly visual drive to SST inter[MASKED_JOURNAL]s. Overall,
the responses of SST inter[MASKED_JOURNAL]s to mismatch were diverse
(Figure S3E), indicating that SST expression does not mark
one homogeneous functional class of inter[MASKED_JOURNAL]s.
...
189 [MASKED_JOURNAL]s; NT: three mice, 137 [MASKED_JOURNAL]s). Given that VIP inter-
[MASKED_JOURNAL]s receive direct inhibitory input from SST inter[MASKED_JOURNAL]s
...
rons (Fu et al., 2014) and a relief from SST inter[MASKED_JOURNAL]-mediated
inhibition. Interestingly, running-related input to VIP inter[MASKED_JOURNAL]s
was strongly experience dependent. VIP inter[MASKED_JOURNAL]s were
...
tion of running-onset responses in SST inter[MASKED_JOURNAL]s in darkness,
a running-related input to VIP inter[MASKED_JOURNAL]s in NT mice was

Figure 3. SST Inter[MASKED_JOURNAL]s Are Strongly Driven by Visual Flow
Average correlation of neural activity with visual flow during open-loop ses-
sions for excitatory [MASKED_JOURNAL]s (average correlations: CT: 0.00, NT: 0.01) and
SST (CT: 0.13, NT: 0.04), VIP (CT: 0.01, NT: 0.03), PV (CT: 0.01, NT: 0.00),
and NPY (CT: 0.02, NT: 0.02) inter[MASKED_JOURNAL]s in CT and NT mice. Average cor-
relation of activity with visual flow was highest for SST inter[MASKED_JOURNAL]s. Error bars
indicates SEM. *p < 0.05, **p < 0.01, ***p < 0.001, n.s., not significant, p R 0.05,
Student's t test.

> Figure caption (from PDF text): Figure 3. SST Inter[MASKED_JOURNAL]s Are Strongly Driven by Visual Flow
Average correlation of neural activity with visual flow during open-loop ses-
sions for excitatory [MASKED_JOURNAL]s (average correlations: CT: 0.00, NT: 0.01) and
SST (CT: 0.13, NT: 0.04), VIP (CT: 0.01, NT: 0.03), PV (CT: 0.01, NT: 0.00),
and NPY (CT: 0.02, NT: 0.02) inter[MASKED_JOURNAL]s in CT and NT mice. Average cor-
...
This figure is a set of comparative bar/point plots illustrating the average correlation between neural activity and visual flow across different types of inter[MASKED_JOURNAL]s in two experimental conditions, CT and NT.

**1. Overall Layout & Structure:**
The figure is structured as a single, multi-series scatter plot with error bars. The x-axis represents different types of neural populations (inter[MASKED_JOURNAL]s and excitatory [MASKED_JOURNAL]s), while the y-axis quantifies the correlation coefficient. The data points are differentiated by color and shape to represent the two experimental conditions (CT and NT).
...
The caption confirms the identity of the plotted populations: SST, Excitatory [MASKED_JOURNAL]s, VIP, PV, and NPY inter[MASKED_JOURNAL]s. It specifies that the correlations shown are for "open-loop sessions." The caption further clarifies the specific average correlation values:
*   Excitatory [MASKED_JOURNAL]s (CT: 0.00, NT: $\approx -0.01$)
*   SST (CT: 0.13, NT: 0.04)
*   VIP (CT: $\approx -0.01$, NT: $\approx -0.03$)
*   PV (CT: $\approx -0.01$, NT: 0.00)
*   NPY (CT: 0.02, NT: 0.02)

The caption also notes that the correlation was highest for SST inter[MASKED_JOURNAL]s, which is visually supported by the highest positive value (CT: 0.13). The statistical notations ($\text{*}$, $\text{**}$, $\text{***}$) correspond to $p < 0.05$, $p < 0.01$, and $p < 0.001$, respectively, based on a Student's t test.
...
suggest that the inhibitory connection from SST inter[MASKED_JOURNAL]s
onto VIP inter[MASKED_JOURNAL]s is stronger in absence of visuomotor
experience.

Finally, responses in both PV inter[MASKED_JOURNAL]s (CT: five mice, 498
[MASKED_JOURNAL]s; NT: six mice, 344 [MASKED_JOURNAL]s) and NPY inter[MASKED_JOURNAL]s
(CT: three mice, 189 [MASKED_JOURNAL]s; NT: three mice, 137 [MASKED_JOURNAL]s)
were behavioral state and visuomotor-experience dependent.
These two inter[MASKED_JOURNAL] subtypes were activated by mismatch
...
tion of the excitatory [MASKED_JOURNAL] population in CT mice in response
to mismatch (Figure 1E). Either excitatory [MASKED_JOURNAL]s recruit PV
and NPY inter[MASKED_JOURNAL]s only above a given activity level or the cal-
cium dynamics in PV and NPY inter[MASKED_JOURNAL]s are such that we are
...
tion response of excitatory [MASKED_JOURNAL]s to running onset is smaller
than that to mismatch (Figures 1E and 1F), but the running-onset
response of PV inter[MASKED_JOURNAL]s is larger than that to mismatch (Fig-
ures 4E and 4F). One potential consequence of a selective acti-
vation of PV inter[MASKED_JOURNAL]s in CT mice is that the PV activation
could lead to a response normalization in excitatory [MASKED_JOURNAL]s
...
Our data indicate that layer 2/3 excitatory mismatch [MASKED_JOURNAL]s
and a subset of VIP inter[MASKED_JOURNAL]s receive excitatory, motor-
related input, while a subset of SST inter[MASKED_JOURNAL]s is more strongly
driven by visual flow. Consistent with the finding that SST inter-
[MASKED_JOURNAL]s receive strong input from surrounding excitatory neu-
rons (Adesnik et al., 2012; Fino and Yuste, 2011; Jiang et al.,
2015), we found that excitatory [MASKED_JOURNAL]s whose activity corre-
lates positively with visual flow (CT: 24% or 539 of 2,259 of neu-
rons; NT: 24% or 513 of 2,104 [MASKED_JOURNAL]s) exhibit a decrease in
activity on mismatch similar to SST inter[MASKED_JOURNAL]s (Figure S1J).
Based on the connectivity motif of excitatory [MASKED_JOURNAL]s, SST and
VIP inter[MASKED_JOURNAL]s (Pfeffer et al., 2013; Pi et al., 2013), we propose
a schematic model circuit to explain mismatch responses in
layer 2/3 excitatory [MASKED_JOURNAL]s (Figure 5A). SST inter[MASKED_JOURNAL]s target
the apical dendrites of layer 2/3 excitatory [MASKED_JOURNAL]s (Markram
et al., 2004). A reduction of visual input onto SST inter[MASKED_JOURNAL]s
...
[MASKED_JOURNAL]. Based on this model, we predict that both SST inter-
[MASKED_JOURNAL] activation and inhibition should lead to a decrease of
the mismatch response in excitatory [MASKED_JOURNAL]s but should have
opposing effects on running-related activity in excitatory neu-
rons (Figure 5B). To test this, we pharmacogenetically manipu-
lated the activity of SST inter[MASKED_JOURNAL]s using designer receptors
...
transfected [MASKED_JOURNAL]s with GCaMP6f to record mismatch and
running related activity in putative excitatory [MASKED_JOURNAL]s. Note
that in these experiments we cannot exclude the possibility
that some of these putative excitatory [MASKED_JOURNAL]s are non-SST

Figure 4. Experience-Dependent Visuomo-
tor Integration in Inhibitory Inter[MASKED_JOURNAL]s
(A) Average population responses to mismatch
(solid line) and playback halt (dashed line) for SST
inter[MASKED_JOURNAL]s from CT (blue, five mice, 118 neu-
rons) and NT (red, five mice, 157 [MASKED_JOURNAL]s) mice.
For both CT and NT mice, SST inter[MASKED_JOURNAL]s re-
...
ter[MASKED_JOURNAL]s (CT: three mice, 189 [MASKED_JOURNAL]s; NT: three
mice, 137 [MASKED_JOURNAL]s). VIP inter[MASKED_JOURNAL]s responded with an increase in activity independent of experience but did not respond to playback halt.
(E and F) Same as in (A and B) but for PV inter[MASKED_JOURNAL]s (CT: five mice, 498 [MASKED_JOURNAL]s; NT: six mice, 344 [MASKED_JOURNAL]s). The mismatch response in PV inter[MASKED_JOURNAL]s was
strongly experience dependent.
See also Figure S3.

## 

inter[MASKED_JOURNAL]s. We found that DREADD inhibition of SST interneu-
rons led to an increase in running-related activity in excitatory
[MASKED_JOURNAL]s, while DREADD activation of SST inter[MASKED_JOURNAL]s led to a
decrease in running-related activity (Figure 5C). In addition,
both inhibition and activation of SST inter[MASKED_JOURNAL]s led to a
decrease in the mismatch response of excitatory [MASKED_JOURNAL]s (Fig-
ures 5D and 5E). These results are consistent with a model of
mismatch computation in which mismatch responses in layer
2/3 [MASKED_JOURNAL]s are the result of a relief of SST inter[MASKED_JOURNAL]-mediated
...
mismatch [MASKED_JOURNAL]s based on their responses to mismatch events
in closed-loop sessions (in the following simply referred to as
mismatch [MASKED_JOURNAL]s) and measured the responses of these neu-
rons to brief (1 s) activation or inhibition of SST or VIP interneu-
rons (see STAR Methods). We found that activation of SST
inter[MASKED_JOURNAL]s resulted in an inhibition of mismatch [MASKED_JOURNAL]s that
was strong enough to fully suppress mismatch responses in
mismatch [MASKED_JOURNAL]s when SST inter[MASKED_JOURNAL]s were activated
concurrently with a mismatch event (Figure 6A). Consistent
with this, inhibition of SST [MASKED_JOURNAL]s resulted in an activation of
mismatch [MASKED_JOURNAL]s and concurrent inhibition of SST inter[MASKED_JOURNAL]s
with a mismatch event resulted in increased mismatch re-
sponses (Figure 6B). Conversely, activation of VIP inter[MASKED_JOURNAL]s
resulted in an activation of mismatch [MASKED_JOURNAL]s and an increase
of the mismatch response when VIP inter[MASKED_JOURNAL]s were activated

Figure 5. A Drop in SST Activity Leads to a Mismatch Response in Excitatory [MASKED_JOURNAL]s
(A) Schematic of a model circuit describing the computation of mismatch responses in layer 2/3 of V1. Excitatory [MASKED_JOURNAL]s and VIP inter[MASKED_JOURNAL]s receive excitatory
motor-related input (purple arrow; dashed purple line depicts idealized running profile around a mismatch, indicated by orange shading). SST inter[MASKED_JOURNAL]s
receive visual flow input (green arrow; dashed green line depicts idealized visual flow around a mismatch, indicated by orange shading). Blue lines next to [MASKED_JOURNAL]s
depict average mismatch responses of excitatory [MASKED_JOURNAL]s (Figure 1E), SST (Figure 4A), and VIP (Figure 4C) inter[MASKED_JOURNAL]s from CT mice. During mismatch, visual
flow is halted and the activity of SST inter[MASKED_JOURNAL]s decreases, thereby disinhibiting the apical dendrites of mismatch [MASKED_JOURNAL]s and allowing the excitatory motor-
related input to activate the [MASKED_JOURNAL]. VIP inter[MASKED_JOURNAL]s amplify this effect by further suppressing SST inter[MASKED_JOURNAL] activity.
(B) Predicted effects of pharmacogenetic manipulation of SST inter[MASKED_JOURNAL]s on excitatory [MASKED_JOURNAL]s. Idealized activity profiles of excitatory motor-related activity
(purple line) and SST inter[MASKED_JOURNAL] activity for a short period of running during a closed-loop session including a mismatch (onset marked by vertical line). In normal
conditions (top), SST inter[MASKED_JOURNAL] activity balances the motor-related input and the mismatch response of excitatory [MASKED_JOURNAL]s is maximal (mismatch-triggered
difference between excitatory and inhibitory input, orange shading). Inhibition of SST inter[MASKED_JOURNAL]s (middle) should result in a smaller mismatch-induced dif-
ference in inhibition and therefore a smaller mismatch response as well as increased running-related activity. Excitation of SST inter[MASKED_JOURNAL]s (bottom) should also
result in smaller mismatch responses due to an over-inhibition of excitatory [MASKED_JOURNAL]s but decreased running-related activity.
...
(left; 829 [MASKED_JOURNAL]s, ***p < 0.001, Wilcoxon signed-rank test) or an excitatory (right; 411 [MASKED_JOURNAL]s, ***p < 0.001, Wilcoxon signed-rank test) DREADD in SST
inter[MASKED_JOURNAL]s. Error bars indicate SEM.
(D) Average population mismatch responses of excitatory [MASKED_JOURNAL]s before (green trace) and 30 min after (yellow trace) the injection of CNO in mice expressing an
inhibitory DREADD in SST inter[MASKED_JOURNAL]s (four mice, 829 [MASKED_JOURNAL]s). Orange bar indicates duration of mismatch; shading indicates SEM. Statistical comparisons as
in Figure 1E.
(E) Same as in (D), but for mice expressing an excitatory DREADD in SST inter[MASKED_JOURNAL]s (two mice, 411 [MASKED_JOURNAL]s).
See also Figure S7.

> Figure caption (from PDF text): Figure 5. A Drop in SST Activity Leads to a Mismatch Response in Excitatory [MASKED_JOURNAL]s
(A) Schematic of a model circuit describing the computation of mismatch responses in layer 2/3 of V1. Excitatory [MASKED_JOURNAL]s and VIP inter[MASKED_JOURNAL]s receive excitatory
motor-related input (purple arrow; dashed purple line depicts idealized running profile around a mismatch, indicated by orange shading). SST inter[MASKED_JOURNAL]s
receive visual flow input (green arrow; dashed green line depicts idealized visual flow around a mismatch, indicated by orange shading). Blue lines next to [MASKED_JOURNAL]s
depict average mismatch responses of excitatory [MASKED_JOURNAL]s (Figure 1E), SST (Figure 4A), and VIP (Figure 4C) inter[MASKED_JOURNAL]s from CT mice. During mismatch, visual
...
This figure, titled "A Drop in SST Activity Leads to a Mismatch Response in Excitatory [MASKED_JOURNAL]s," is composed of five distinct panels (A, B, C, D, and E), illustrating a computational model and experimental results related to mismatch responses in V1.

## 

concurrently with a mismatch event (Figure 6C). Finally, inhibition
of VIP inter[MASKED_JOURNAL]s resulted in an inhibition of mismatch [MASKED_JOURNAL]s
that was strong enough to suppress mismatch responses (Fig-
ure 6D). Note that even though these effects were stronger for
mismatch [MASKED_JOURNAL]s than for putative excitatory [MASKED_JOURNAL]s that did
not respond to mismatch (Figure S4), it is very likely only a subset
of SST and VIP inter[MASKED_JOURNAL]s that are part of the circuit involved in
mismatch responses in excitatory [MASKED_JOURNAL]s. In summary, these
...
mismatch [MASKED_JOURNAL]s under inhibitory control of SST inter[MASKED_JOURNAL]s.
...
motor mismatch responses in layer 2/3 excitatory [MASKED_JOURNAL]s.
...
found that mismatch responses of excitatory [MASKED_JOURNAL]s in CT
and NT mice equalized rapidly with normal visuomotor experi-
ence (Figures 7A-7C). The population mismatch responses of
SST and VIP inter[MASKED_JOURNAL]s remained stable throughout the
course of the experiment for both CT and NT mice (Figures 7D

Figure 6. Mismatch [MASKED_JOURNAL]s Are Inhibited by SST Activation or VIP Inhibition and Activated by SST Inhibition or VIP Activation
(A) Left: schematic of the experimental design. ChrimsonR was selectively expressed in SST inter[MASKED_JOURNAL]s and GCaMP6f in all [MASKED_JOURNAL]s. We then locally activated
SST inter[MASKED_JOURNAL]s through the imaging objective while imaging GCaMP6f activity in all [MASKED_JOURNAL]s. Right: response of putative excitatory mismatch-responsive
[MASKED_JOURNAL]s (165 [MASKED_JOURNAL]s, five mice) to mismatch (green line), optogenetic activation of SST inter[MASKED_JOURNAL]s during running (purple line), and concurrent mismatch and
optogenetic activation of SST inter[MASKED_JOURNAL]s (yellow line). Orange area indicates duration of mismatch and duration of optogenetic stimulation respectively;
...
(B) Left: as in (A), but expressing ArchT in SST inter[MASKED_JOURNAL]s. Right: responses of mismatch [MASKED_JOURNAL]s (236 [MASKED_JOURNAL]s, four mice) as in (A), but for optogenetic inhibition
of SST inter[MASKED_JOURNAL]s.
(C) Left: as in (A), but expressing ChrimsonR in VIP inter[MASKED_JOURNAL]s. Right: responses of mismatch [MASKED_JOURNAL]s (114 [MASKED_JOURNAL]s, four mice) as in (A), but for optogenetic
activation of VIP inter[MASKED_JOURNAL]s.
(D) Left: as in (A), but expressing ArchT in VIP inter[MASKED_JOURNAL]s. Right: responses of mismatch [MASKED_JOURNAL]s (107 [MASKED_JOURNAL]s, three mice) as in (A), but for optogenetic inhibition
of VIP inter[MASKED_JOURNAL]s. Error bars indicate SEM.
See also Figure S4.

> Figure caption (from PDF text): Figure 6. Mismatch [MASKED_JOURNAL]s Are Inhibited by SST Activation or VIP Inhibition and Activated by SST Inhibition or VIP Activation
(A) Left: schematic of the experimental design. ChrimsonR was selectively expressed in SST inter[MASKED_JOURNAL]s and GCaMP6f in all [MASKED_JOURNAL]s. We then locally activated
SST inter[MASKED_JOURNAL]s through the imaging objective while imaging GCaMP6f activity in all [MASKED_JOURNAL]s. Right: response of putative excitatory mismatch-responsive
[MASKED_JOURNAL]s (165 [MASKED_JOURNAL]s, five mice) to mismatch (green line), optogenetic activation of SST inter[MASKED_JOURNAL]s during running (purple line), and concurrent mismatch and
optogenetic activation of SST inter[MASKED_JOURNAL]s (yellow line). Orange area indicates duration of mismatch and duration of optogenetic stimulation respectively;
...
This figure, titled "Mismatch [MASKED_JOURNAL]s Are Inhibited by SST Activation or VIP Inhibition and Activated by SST Inhibition or VIP Activation," is composed of four main panels (A, B, C, and D), each presenting a combination of schematic diagrams and corresponding time-course plots.
...
**Left Schematic:**
*   This panel shows a schematic diagram illustrating the experimental setup. It depicts two [MASKED_JOURNAL]al types: **SST** (Somatostatin-expressing inter[MASKED_JOURNAL]s) and **VIP** (Vasoactive Intestinal Peptide [MASKED_JOURNAL]s).
*   The SST [MASKED_JOURNAL] is shown with a schematic representation of the genetic tools: **SST-Cre mouse**, **AAV-Ef1a-DIO-ChrimsonR** (targeting SST [MASKED_JOURNAL]s), and **AAV-Ef1a-GCaMP6f**.
*   The diagram shows the SST [MASKED_JOURNAL] connected to a general [MASKED_JOURNAL]al population (implied by the context of GCaMP6f expression).
...
**Left Schematic:**
*   This schematic is similar to Panel A's left side, showing SST and VIP [MASKED_JOURNAL]s.
*   The genetic tools are specified as: **SST-Cre mouse**, **AAV-CAG-FLex-ArchT** (targeting SST [MASKED_JOURNAL]s), and **AAV-Ef1a-GCaMP6f**.
...
**Left Schematic:**
*   This schematic shows SST and VIP [MASKED_JOURNAL]s.
*   The genetic tools are specified as: **VIP-Cre mouse**, **AAV-Ef1a-DIO-ChrimsonR** (targeting VIP [MASKED_JOURNAL]s), and **AAV-Ef1a-GCaMP6f**.
...
**Left Schematic:**
*   This schematic shows SST and VIP [MASKED_JOURNAL]s.
*   The genetic tools are specified as: **VIP-Cre mouse**, **AAV-CAG-FLex-ArchT** (targeting VIP [MASKED_JOURNAL]s), and **AAV-Ef1a-GCaMP6f**.
*   The presence of ArchT suggests optogenetic inhibition.
...
response of VIP and SST inter[MASKED_JOURNAL]s developed independent
of visuomotor coupling. Similar to excitatory [MASKED_JOURNAL]s, mismatch

responses in PV and NPY inter[MASKED_JOURNAL]s equalized after restora-
...
(A) Average responses to mismatch (solid lines) and playback halt (dashed lines) of [MASKED_JOURNAL]s with positive correlation of activity with running speed (running
...
[MASKED_JOURNAL]s per mouse, nine mice; NT: 10% ± 3%, nine mice). Orange area indicates duration of mismatch; shading indicates SEM. Statistical comparison as in
Figure 1E.
(B) Same as (A), but for last imaging day (CT: 10% ± 2% of [MASKED_JOURNAL]s per mouse, eight mice; NT: 9% ± 1%, seven mice).
(C) Average responses to mismatch and playback halt (see STAR Methods) of excitatory [MASKED_JOURNAL]s selected as in (A) as a function of imaging days for CT and NT
...
(D) Average population responses to mismatch of SST inter[MASKED_JOURNAL]s, as a function of imaging days for CT and NT mice (CT, five mice, 118 [MASKED_JOURNAL]s; NT: five mice,
157 [MASKED_JOURNAL]s). Statistical test as in (C). Error bars indicate SEM.
(E) As in (D) but for VIP inter[MASKED_JOURNAL]s (CT: three mice, 189 [MASKED_JOURNAL]s; NT: three mice, 137 [MASKED_JOURNAL]s).
(F) As in (D) but for PV inter[MASKED_JOURNAL]s (CT: five mice, 498 [MASKED_JOURNAL]s; NT: six mice, 344 [MASKED_JOURNAL]s).
...
input in mismatch [MASKED_JOURNAL]s. SST inter[MASKED_JOURNAL]s mediate the inhibi-
tion by visual flow, while mutual inhibition between VIP and SST
inter[MASKED_JOURNAL]s (Pfeffer et al., 2013) acts to amplify the responses of
SST inter[MASKED_JOURNAL]s. Even though the average response of SST in-
ter[MASKED_JOURNAL]s to mismatch is a decrease in activity, this effect is
carried by only a subset of SST inter[MASKED_JOURNAL]s. Moreover, although
such a simplified model is sufficient to explain mismatch re-
sponses, the interactions between the different inter[MASKED_JOURNAL]
subtypes are in all likelihood much richer than schematically
summarized here. PV inter[MASKED_JOURNAL]s, for example, could act to
normalize the mismatch response in excitatory [MASKED_JOURNAL]s (Hofer
et al., 2011; Kerlin et al., 2010).

Given that SST inter[MASKED_JOURNAL]s provide visual inhibition to excit-
atory mismatch [MASKED_JOURNAL]s and that mismatch responses in SST in-
ter[MASKED_JOURNAL]s do not depend on visuomotor experience (Figure 4A),
it is likely that visuomotor experience predominantly modifies the
synaptic inputs onto the excitatory [MASKED_JOURNAL]. In this way, a balance
...
by PV inter[MASKED_JOURNAL]s (Xue et al., 2014). Note that the type of
mismatch [MASKED_JOURNAL] we describe here balances an excitatory
...
[MASKED_JOURNAL] balances an inhibitory top-down input against an excit-
atory feedforward input, it would signal a visual input that is
stronger than predicted. Such a [MASKED_JOURNAL] would have classic visual
...
B Extraction of [MASKED_JOURNAL]al activity

B Data analysis

B Modified leaky integrate-and-fire [MASKED_JOURNAL] model
