...
could be falsiﬁed experimentally. Lastly, we summarize the general implications of predictive processing on
...
would extend to one of his most inﬂuential concepts—the
receptive ﬁeld—paving the way for a view of the brain that is
driven to move by its sensorium. Sherrington’s views of a ner-
vous system built upon sensory-driven receptive ﬁelds would
ﬂourish over the next several decades. Describing the re-
...
sion that these neurons function as ﬂy detectors (Barlow, 1953).
...
principle for scientiﬁc inquiry; it is apparent not only in the
concept of receptive ﬁelds of neurons in visual cortex, but
...
1950), and Roger Sperry (Sperry, 1950). They were unsatisﬁed
...
able to make this distinction is the fact that the optokinetic reﬂex
does not prevent self-motion of the eye. During passive viewing,
full-ﬁeld visual ﬂow results in a movement of the eye that stabilizes
the image on the retina; this is called the optokinetic reﬂex. If the
animal could not distinguish between self-generated and exter-
nally generated visual input, then the optokinetic reﬂex would pre-
vent any active movement of the eye. The argument is that the
visual ﬂow resulting from an eye movement would trigger the
optokinetic reﬂex just as visual ﬂow during passive viewing
does and thus would result in a reﬂexive eye movement that
...
standing brain has a long tradition in the ﬁelds of computational
...
level. This is in part due to the difﬁculty of designing experiments
...
selective for speciﬁc stimulus features.

Prediction errors may come in two ﬂavors. The bottom-up input
...
neuron that has a sufﬁciently high basal ﬁring rate. Increases in ac-
...
particularly in layer 2/3, the baseline ﬁring rates of principal neu-
...
tageous because it is efﬁcient; fewer spikes are necessary
...
spike in a top-down prediction. In a ﬁrst approximation, this
means that the total number of spikes (bottom-up and top-
down) remains unchanged. Hence, although there are circum-
stances under which predictive processing can be more efﬁ-
cient, in cortex this is likely not the case if efﬁciency is measured
...
bottom-up and top-down input and can thus be modiﬁed in
...
pears to be built on Gabor ﬁlters of the visual input (such as the
receptive ﬁeld of simple cells), and that of auditory cortex is built
on spectro-temporal ﬁlters of the auditory input. In motor cortex,
...
ture in which the ﬂow of signals is reversed, predictions are sent
...
sual or auditory receptive ﬁelds, is in the form of a convolution
with a receptive ﬁeld. The predictive processing framework dif-
...
P that depends on the top-down input—or, more speciﬁcally, the
prediction of the bottom-up input V.

R = ± ðVðbottom­upÞ­Pðtop­downÞÞ
(2)
...
sual feedback (e.g., optic ﬂow). This assumes that animals learn
how sensory feedback couples to movement with experience. In
a ﬁrst approximation, the representational framework predicts
...
increases unexpectedly, positive prediction-error neurons will ﬁre, activating both the internal representation neurons and the top-down prediction (P) from a
...
and inhibit both internal representation neurons and top-down predictions. Responses of all three neuron types should be inﬂuenced by separate gating signals
...
and inhibit both internal representation neurons and top-down predictions. Responses of all three neuron types should be inﬂuenced by separate gating signals
that modulate response amplitude.
> Figure description (generated): This description is based on the provided image snippet and its associated caption, which refers to Figure 2. Since only a partial view of the figure is provided, this description focuses on the visible elements and integrates the contextual information from the caption.
...
The visible area primarily shows a schematic neural circuit diagram, contained within a large, light-purple rectangular bounding box. This structure represents the core computational unit of the predictive processing model. The caption indicates that this figure likely contains two parts: (A) a schematic of the circuit structure, and (B) schematics of neuron responses. The visible portion strongly resembles the structural schematic described in Panel (A).

### 2. Visual Components & Symbols
The circuit diagram utilizes distinct geometric shapes to represent different types of neurons:

*   **Excitatory Neurons (Triangles):** Represented by solid black triangles ($\triangle$). These nodes are shown receiving input and projecting signals.
*   **Inhibitory Neurons (Circles):** Represented by open white circles ($\circ$). These nodes are shown receiving input and projecting signals.
*   **Connections (Lines & Arrows):** Lines connect the nodes, indicating synaptic connections. The arrows on these lines denote the direction of signal flow (i.e., feedforward or feedback).
*   **Input/Output Signals:** There are external lines entering the top of the schematic area, colored green, suggesting inputs or higher-level signals.

**Specific Connections Visible:**
1.  Multiple lines descend from the top, converging towards a central excitatory neuron (triangle). These represent top-down predictions.
2.  A line originates from the left, feeding into the central excitatory neuron (triangle). This likely represents sensory input.
3.  The central excitatory neuron projects signals downwards and to the right.
4.  There is a connection originating from an inhibitory neuron (circle) on the right, which projects towards another node.
5.  The structure shows complex feedback loops involving the triangles and circles, consistent with the description of prediction error computation.

### 3. Labels, Keys & Legends (Inferred from Caption)
The caption provides the necessary key for interpreting the symbols:

*   **Triangles ($\triangle$):** Represent **excitatory neurons**.
*   **Circles ($\circ$):** Represent **inhibitory neurons**.
*   **Circuit Function (Panel A context):** The structure models the computation of prediction errors.
    *   **Type 1 Neurons (Sensory Input - Prediction [S-P]):** Compute positive prediction errors.
    *   **Type 2 Neurons (Prediction – Sensory Input [P-S]):** Compute negative prediction errors.
*   **Hierarchical Processing:** The schematic assumes a hierarchical structure, though the caption notes that non-hierarchical communication would involve bidirectional signaling.

### 4. Data Trends & Details
No quantitative plots or axes are visible in the provided snippet; therefore, no data trends can be described.

### 5. Contextual Caption Integration (Panel B Inference)
The caption describes Panel (B), which is not fully visible, but it details the dynamic responses:

*   **Positive Prediction Error:** If bottom-up input (S) increases unexpectedly, positive prediction-error neurons fire, activating internal representation neurons and top-down predictions (P). This activation subsequently inhibits the positive prediction-error neuron.
*   **Negative Prediction Error:** If bottom-up input (S) decreases, negative prediction-error neurons activate and inhibit both internal representation neurons and top-down predictions.
*   **Modulation:** The responses of all three neuron types are influenced by separate gating signals that modulate response amplitude.
...
round of the classical receptive ﬁeld is the consequence of top-
...
inhibits responses of neurons with receptive ﬁelds in neighboring
parts of the visual ﬁeld to the same stimulus. In layer 2/3 of
...
tive ﬁeld properties can be explained in a predictive processing
...
(Haider et al., 2013). Consistent with a strong top-down inﬂu-
...
where responses were quantiﬁed to unexpected changes in the
...
difﬁcult to explain in a representation framework is a response to
...
selectively to the absence of expected visual ﬂow (Keller et al.,
...
in sensory cortices are expected to be feature-speciﬁc and not
...
between predicted and actual visual ﬂow in spatially conﬁned
areas of the visual ﬁeld (Zmarz and Keller, 2016). These mismatch
...
2014; Polack et al., 2013), which exert context-dependent inﬂu-
...
veys an experience-dependent prediction of visual ﬂow (rather
...
predictions of full-ﬁeld visual ﬂow. In a learned coupling between
...
reciprocally, it is the interaction with the world that reﬁnes these
...
(i.e., those that ﬁre together) can preferentially link up into synap-
...
works across cortical areas, whereby a history of correlated ﬁring
...
coupling, predictions of visual ﬂow are learned (Attinger et al.,
...
puts whose inﬂuence is shaped by experience.
...
ally, a dynamic modulation of the inﬂuence of top-down and
...
ance of top-down versus bottom-up inﬂuence (Yu and Dayan,
2005). Speciﬁcally, the neuromodulatory tone may shift the rela-
tive contribution of bottom-up and top-down signals such that
the inﬂuence of prediction errors can be modulated according
...
course, we do not have a deﬁnitive answer to this question, but
...
postulate that perception is based on a ﬁnely tuned process that
...
an internal model with sufﬁcient predictive capacity, a simple
...
In this section, we outline experiments that may test, reﬁne, or
...
way, one could test the inﬂuence of activation of a subset of pu-
...
on functionally identiﬁed prediction-error neurons in layer 2/3.

(2) Assuming that cortex is built based on a canonical circuit
motif (Douglas et al., 1989), we should ﬁnd prediction-error neu-
...
a canonical cortical computation, we should ﬁnd similar predic-
...
animal expects to encounter in its conspeciﬁcs. There is some ev-
...
preventing experience-dependent plasticity in speciﬁc cell types
...
learning, for example, listening to conspeciﬁc vocalizations should
...
may beatleast three deﬁning characteristics ofsucha signal. First,
...
are either too strong or too weak, we would expect to ﬁnd a com-
...
It is highly probable that the inﬂuence of cortex was initially
...
sensory input. One strategy to expand behavioral ﬂexibility is
...
the ever increasingly complex behavior of our conspeciﬁcs—or
