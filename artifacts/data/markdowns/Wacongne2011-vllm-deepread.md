...
viance of the entire sequence and also allowed us to probe whether the
omission effect differed when a standard or a deviant tone was expected.
> Figure description (generated): This figure, labeled "Fig. 1," illustrates the experimental design involving three types of auditory stimuli presented in different block structures. The overall layout is divided into three distinct sections, corresponding to the three stimulus types: "xxxxx block," "xxxxY block," and "Omission block."
...
and sustained response to global deviance. The source in precental cortex shows only the late sustained response. Both effects are present on omission trials
(red/pink curves), with greater initial response to omissions in xxxxY blocks (thin red curve) than in xxxxx blocks (thick pink curve).
> Figure description (generated): This figure, labeled Figure 3, presents source modeling results across four panels (A, B, C, and D), illustrating neural activity related to different experimental conditions.
