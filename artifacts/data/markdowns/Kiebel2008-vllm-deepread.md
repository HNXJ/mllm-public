...
Citation: Kiebel SJ, Daunizeau J, Friston KJ (2008) A Hierarchy of Time-Scales and the Brain. PLoS Comput Biol 4(11): e1000209. doi:10.1371/journal.pcbi.1000209
...
> Figure description (generated): Since no figure was provided, I cannot generate the detailed description.
...
### Panel A: Data and States Dynamics

Panel A is structured into two rows (representing the 1st level and 2nd level) and two columns (representing "Data" and "Hidden states").

**Top Row (1st level):**
This row contains two plots, both spanning a time duration from $s=0$ to $s=2$.

1.  **Top-Left Plot (Data):**
    *   **Title:** "Data"
    *   **Y-axis Label:** "$y$" (with units "a.u.")
    *   **X-axis Label:** "$s$" (time)
    *   **Content:** This plot shows time-series data. There are two solid lines: a blue line and a green line, oscillating around zero. Additionally, there are red dotted lines near the top of the plot area, which the caption identifies as indicating "observation error on the output." The blue and green lines exhibit complex, fluctuating patterns.

2.  **Top-Right Plot (Hidden states):**
    *   **Title:** "Hidden states"
    *   **Y-axis Label:** "$\chi^{(1)}$" (with units "a.u.")
    *   **X-axis Label:** "$s$" (time)
    *   **Content:** This plot displays three solid lines: blue, green, and red. These represent the hidden states of a Lorenz attractor at the first level. The lines show distinct, complex oscillatory behavior over time $s$.

**Bottom Row (2nd level):**
This row also contains two plots, spanning the same time duration ($s=0$ to $s=2$).

1.  **Bottom-Left Plot (Causal States):**
    *   **Title:** "Causal States"
    *   **Y-axis Label:** "$v^{(1)}$" (with units "a.u.")
    *   **X-axis Label:** "$s$" (time)
    *   **Content:** This plot shows a single blue solid line, representing the causal state. The curve starts high (around 30 a.u.) and exhibits a smooth, wave-like trajectory before decreasing towards the end of the time window.

2.  **Bottom-Right Plot (Hidden states):**
    *   **Title:** "Hidden states"
    *   **Y-axis Label:** "$\chi^{(2)}$" (with units "a.u.")
    *   **X-axis Label:** "$s$" (time)
    *   **Content:** This plot displays three solid lines: blue, green, and red. These represent the hidden states at the second level. The dynamics here are slower compared to the first level, as noted in the caption.

### Panel B: Sonogram

Panel B is a single plot located to the right of Panel A.

*   **Title/Context:** The caption identifies this as a "Sonogram (time-frequency representation) constructed from model output."
*   **Y-axis Label:** "Hz" (Frequency in Hertz)
*   **X-axis Label:** "$s$" (Time)
*   **Content:** This plot is a dark background with bright, vertical streaks of high intensity. These streaks represent time-frequency locations with greater power, characteristic of a sonogram showing patterned vocalizations. The frequency range shown spans from approximately 2000 Hz to 5000 Hz, and the time axis spans from $s=0$ to $s=2$.
...
u tzDt
ð
Þ~u tð Þzu’ tð ÞDtz1=2u’’ tð ÞDt2z1=6u’’’ tð ÞDt3z . . . , ð6Þ
...
the prediction error time series of both levels. See text for their simulation. Red: LFPs at first level, dark red: LFP at second level.
doi:10.1371/journal.pcbi.1000209.g002
> Figure description (generated): ## Figure Description: Dynamic Online Inversion
...
The figure is structured into multiple subplots:
*   **Panel A:** Contains four distinct time-series plots arranged in a 2x2 grid.
*   **Panel B:** Contains one time-series plot located below Panel A.

### Visual Components & Symbols (Panel A)

**Top Left Plot: Predictions (1st level)**
*   This plot shows time-series data over a horizontal axis labeled 's' (time).
*   The vertical axis is labeled 'a.u.' (arbitrary units) and ranges from -20 to 50.
*   It displays multiple lines: a solid green line, a solid blue line, and shaded grey areas.
*   The caption indicates that the **solid, coloured lines** represent model predictions, and the **grey shaded area** represents the 90% confidence interval around the conditional means.

**Top Right Plot: Hidden states (1st level)**
*   This plot also uses 's' on the x-axis and 'a.u.' on the y-axis (ranging from -20 to 50).
*   It displays solid lines, including a prominent red line labeled $\chi^{(1)}$ and other colored lines.
*   The caption specifies that the **red dotted lines** indicate the prediction error (difference between observation and model prediction).

**Bottom Left Plot: Causal States (2nd level)**
*   This plot uses 's' on the x-axis and 'a.u.' on the y-axis (ranging from -10 to 50).
*   It shows a solid blue line and a grey shaded area.
*   The caption identifies the **grey shaded area** as the 90% confidence interval.

**Bottom Right Plot: Hidden states (2nd level)**
*   This plot uses 's' on the x-axis and 'a.u.' on the y-axis (ranging from -10 to 50).
*   It displays solid lines, including a prominent red line labeled $\chi^{(2)}$ and other colored lines.
*   Again, the caption notes that **red dotted lines** indicate prediction error.

### Visual Components & Symbols (Panel B)

**Bottom Plot: Prediction error (LFP)**
*   This plot shows time-series data over 's' on the x-axis (ranging from 0 to 2).
*   The vertical axis is labeled "Prediction error (LFP)" and ranges from -6 to 6.
*   It displays multiple fluctuating lines: a red line and a dark red line, representing simulated Local Field Potentials (LFPs).

### Labels, Keys & Legends
**Axes Labels:**
*   X-axis for all plots in Panel A: 's' (time).
*   Y-axes for Panel A: 'a.u.' (arbitrary units) or specific ranges thereof.
*   Y-axis for Panel B: "Prediction error (LFP)".

**Notations and Variables:**
*   $\chi^{(1)}$: Appears in the top right plot.
*   $\chi^{(2)}$: Appears in the bottom right plot.

**Annotations (from Caption):**
*   "Observed data... are now shown as black, dotted lines." (Implied in the context of the plots).
*   "Model predictions as solid, coloured lines."
*   "The 90% confidence interval around the conditional means is shown in grey."
*   "The prediction error (i.e., difference between observation and model prediction) is indicated by red dotted lines."
*   Panel B legend interpretation: "Red: LFPs at first level, dark red: LFP at second level."

### Data Trends & Details
*   **Panel A (Predictions/States):** The plots show dynamic, oscillatory behavior across the time axis 's'. In the top left plot (Predictions, 1st level), the green and blue lines exhibit clear periodic fluctuations. The grey confidence intervals track these predictions.
*   **Panel B (Prediction Error LFP):** This plot shows high-frequency, noisy fluctuations centered around zero. The red and dark red lines demonstrate the simulated LFP activity resulting from the prediction error time series at the first and second levels, respectively.
...
doi:10.1371/journal.pcbi.1000209.g003

Hierarchy of Time-Scales

> Figure description (generated): This figure is composed of three distinct panels: Panel A, Panel B, and Panel C.
...
### Overall Layout & Structure
The figure is organized into a $2 \times 2$ grid structure. Panels A and B are positioned on the left side, representing "Predictions," while panels C and D are positioned on the right side, representing simulated LFP data.

### Panel Details

**Panel A: Predictions (No Song Interruption)**
*   **Structure:** This panel contains two subplots arranged side-by-side. The left subplot is labeled "Predictions," and the right subplot is labeled "Hidden states."
*   **X-axis:** Labeled in seconds (s), ranging from 0 to 2.
*   **Y-axis:** Labeled "a.u." (arbitrary units), ranging from -20 to 50 for the left plot, and -20 to 50 for the right plot.
*   **Curves:**
    *   In the "Predictions" subplot, there are multiple colored lines (red, blue, gray) showing fluctuating signals over time. The red line shows a clear upward trend peaking around $t=1.5$ s, while the blue and gray lines show more oscillatory behavior.
    *   In the "Hidden states" subplot, there are corresponding colored lines (red, blue, gray) showing dynamic changes. The red line exhibits a strong upward trend peaking around $t=1.5$ s, mirroring the prediction plot.

**Panel B: Predictions (With Song Interruption)**
*   **Structure:** Similar to Panel A, this panel contains two subplots: "Predictions" on the left and "Hidden states" on the right.
*   **X-axis:** Labeled in seconds (s), ranging from 0 to 2.
*   **Y-axis:** Labeled "a.u.", ranging from -20 to 50 for both subplots.
*   **Curves:**
    *   In the "Predictions" subplot, signals are visible until approximately $t=1.4$ s, after which the lines rapidly approach and remain near the zero line (0 a.u.).
    *   In the "Hidden states" subplot, signals are visible until approximately $t=1.4$ s, after which the lines also rapidly approach and remain near the zero line (0 a.u.).

**Panel C: Prediction error (LFP) for Panel A**
*   **Structure:** This panel is a single time-series plot.
*   **X-axis:** Labeled in seconds (s), ranging from 0 to 2.
*   **Y-axis:** Labeled "micro-volt," ranging from -6 to 8.
*   **Curves:** A single fluctuating line (likely representing the error signal) is plotted, showing significant positive and negative excursions across the time course.

**Panel D: Prediction error (LFP) for Panel B**
*   **Structure:** This panel is a single time-series plot.
*   **X-axis:** Labeled in seconds (s), ranging from 0 to 2.
*   **Y-axis:** Labeled in arbitrary units (implied, though not explicitly labeled on the axis itself, it corresponds to LFP error), ranging from -15 to 10.
*   **Curves:** A single fluctuating line is plotted, showing dynamic changes in the prediction error.

### Contextual Caption Integration
The caption clarifies the context:
*   **Panel A:** Represents the single-level model explaining data *without* a song interruption.
*   **Panel B:** Represents the single-level model behavior *after* an interruption occurring at $t=1.4$ seconds, where the model quickly approaches zero.
*   **Panel C:** Shows simulated LFPs corresponding to the inversion process shown in Panel A.
*   **Panel D:** Shows simulated LFPs corresponding to the inversion process shown in Panel B.
...
single-level model is unable to predict the data at all.
doi:10.1371/journal.pcbi.1000209.g005
> Figure description (generated): ## Figure 5 Description: Comparison of Single- and Two-Level Model Inversion of High-Noise Birdsong Data
...
### Panel A: Two-Level: Predictions
This top-left panel is a time-series plot titled "Two-level: Predictions."

*   **Axes:**
    *   The x-axis is labeled '$s$' and ranges from 0 to 2.
    *   The y-axis is labeled "a.u." (arbitrary units) and ranges from -20 to 50, marked in increments of 10.
*   **Data Curves:** There are multiple overlaid curves representing the model's predictions:
    *   A prominent, high-amplitude curve (likely representing the primary prediction) shows significant peaks and troughs. This curve is labeled with a superscript '$y$' near its peak around $s=1.25$.
    *   Several other curves are present, showing different patterns of oscillation and amplitude modulation across the time course $s$. Some appear to track the main curve, while others show lower-amplitude variations.

### Panel B: Single-Level: Predictions
This top-right panel is a time-series plot titled "Single-level: Predictions."

*   **Axes:**
    *   The x-axis is labeled '$s$' and ranges from 0 to 2.
    *   The y-axis is labeled "a.u." and ranges from -40 to 50, marked in increments of 10.
*   **Data Curves:** This panel displays several oscillating curves:
    *   A prominent, high-amplitude curve (labeled with a superscript '$y$') shows significant oscillations across the time course $s$.
    *   Several other curves are present, exhibiting different frequencies and amplitudes of oscillation. These curves appear to be less coherent or predictive compared to the patterns in Panel A, as noted by the caption.
...
### Contextual Caption Integration
The caption clarifies the purpose of these plots: "Comparison of single- and two-level model inversion of high-noise birdsong data."
*   Panel A represents the output of the two-level model, which the caption notes "can explain the data relatively well, although it misses the third syllable."
*   Panel B represents the output of the single-level model, which the caption states "is unable to predict the data at all."
*   The bottom panel displays the "causal state of the two-level model," represented by $v^{(1)}$.
...
> Figure description (generated): The provided image is a cropped section of what appears to be a **table**, not a complex figure like a schematic or plot.
...
> Figure description (generated): The provided image is a cropped table, not a complex figure containing diagrams, plots, or schematics. Therefore, the description will focus on the structure and content of this table.
...
> Figure description (generated): The provided image is a **table** detailing the "Time-Scale of Environmental Dynamics."

### 1. Overall Layout & Structure
The figure is structured as a two-column table with a header row and multiple data rows.

### 2. Visual Components & Symbols
The table uses simple text entries within rectangular cells to categorize different time scales. There are no complex graphical elements such as plots, schematics, or flow charts; the structure is purely tabular.

### 3. Labels, Keys & Legends
**Column Headers:**
*   The first column header is: "Time-Scale of Environmental Dynamics".
*   The second column header is partially visible and starts with: "Section Tex[t]".

**Row Content (Time Scales):**
The table lists several distinct time scales:
1.  "Milliseconds to hundreds of milliseconds"
2.  "Tens of milliseconds to seconds"
3.  "Tens of seconds to much longer periods" (This entry appears twice, suggesting a potential repetition or grouping in the original source).
4.  "Very long periods"

**Partial Text Below Table:**
Below the main table structure, there is a fragmented line of text: "t are represented".

### 4. Data Trends & Details
As this is a categorical table, there are no quantitative data trends or axes to describe. The content serves to categorize environmental dynamics into specific temporal ranges.

### 5. Contextual Caption Integration
The table itself functions as a classification tool, mapping specific temporal ranges of environmental dynamics to corresponding sections within the source document (indicated by "Section Tex[t]").
